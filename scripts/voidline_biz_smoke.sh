...
voidline_bin="${VOIDLINE_BIN:-${repo_root}/target/debug/voidline}"

artifact_root="${VOIDLINE_BIZ_ARTIFACT_DIR:-${repo_root}/.tmp/voidline-biz-smoke}"
run_id="$(date +%Y%m%d-%H%M%S)"
tmp_dir="${artifact_root}/${run_id}"
mkdir -p "$tmp_dir"

if [[ ! -x "$voidline_bin" ]]; then
    echo "error: voidline binary not found or not executable: $voidline_bin" >&2
    echo "hint: build it first with 'cargo build -p voidline' or set VOIDLINE_BIN" >&2
    exit 1
fi

export VOIDLINE_DB="$tmp_dir/voidline.sqlite3"

run_log="$tmp_dir/run.txt"

{
    printf 'voidline binary: %s\n' "$voidline_bin"
    printf 'sqlite db: %s\n' "$VOIDLINE_DB"
    printf '\n'
} > "$run_log"
...
log_command() {
    ...
    local output status
    if output="$("$voidline_bin" "$@" 2>&1)"; then
        status=0
    else
        status=$?
    fi
...
echo "voidline biz smoke test passed"
