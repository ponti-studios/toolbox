...
function outputError(error, options = {}) {
  ...
  if (json) {
    ...
    if (error && error.code === "NOT_AUTHORIZED") {
      console.error(
        'Tip: On recent macOS versions this can be set to "Add Only". In System Settings > Privacy & Security > Calendars, click Options… and set "Full Access" for your terminal or calling app.',
      );
    }
  }
}
