...
rootOption(frontmatter.command("migrate"))
  ...
  .action((options: { root: string; schema?: string; write?: boolean; dryRun?: boolean; backup?: boolean; output?: string; excludeDir?: string[] }) => {
    const schema = getSchema(options.schema);
    const targets = filesFrom(options.root).filter((target) =>
      !options.excludeDir?.some((dir) => target.file.split("/").includes(dir)) &&
      !target.file.split("/").some((part) => part.startsWith("_")));
    const results = targets.map((target) => {
      const changes: string[] = [];
      const data = { ...target.data };
      for (const [key, value] of Object.entries(schema.defaults)) if (data[key] === undefined) {
        data[key] = value;
    ...
    if (options.output === "json") console.log(JSON.stringify({ files: results, summary: { processed_files: results.length, changed_files: results.filter((result) => result.changed).length } }, null, 2));
    else {
      for (const result of results.filter((item) => item.changed)) console.log(`${options.write && !options.dryRun ? "Migrated" : "Would migrate"}: ${result.path}`);
      console.log(`Processed: ${results.length}, Changed: ${results.filter((result) => result.changed).length}, Error files: 0`);
    }
  });
...
files.command("find-duplicates").argument("<directory>").option("--algorithm <algorithm>", "md5").option("--min-size <bytes>", "0").option("--extensions <extensions...>").option("--show-hashes")
  .action((directory: string, options: { algorithm: "md5" | "sha1" | "sha256"; minSize: string; extensions?: string[]; showHashes?: boolean }) =>
    findDuplicates(directory, options.algorithm, Number(options.minSize), options.extensions ?? [], Boolean(options.showHashes)));
files.command("bulk-rename").argument("<directory>").requiredOption("--pattern <pattern>").requiredOption("--replacement <replacement>").option("--apply")
  .action((directory: string, options: { pattern: string; replacement: string; apply?: boolean }) => {
...
files.command("convert").argument("<input>").argument("<output>").option("--csv-to-json").option("--no-flatten").option("--separator <separator>", ".")
  .action((input: string, output: string, options: { csvToJson?: boolean; flatten?: boolean; separator: string }) =>
    convert(input, output, Boolean(options.csvToJson), options.flatten !== false, options.separator));
files.command("xlsx-to-csv").option("-f, --file <file>").option("-d, --directory <directory>").option("-a, --all-sheets")
...
program.command("analyze").option("-r, --root <root>", ".").option("-o, --output <output>", "text").option("--files").option("--extensions <extensions>").option("--include-hidden").option("--ignore-file <files...>").option("--no-gitignore")
  .action((options: { root: string; output?: string; files?: boolean; extensions?: string; includeHidden?: boolean; ignoreFile?: string[]; noGitignore?: boolean }) => analyze(options.root, options));

const completions = program.command("completions");
completions.command("generate").argument("<shell>").action((shell: string) => generateCompletions(shell));
completions.command("install").argument("<shell>").option("--dry-run").option("--force")
  .action((shell: string, options: { dryRun?: boolean; force?: boolean }) => installCompletions(shell, options));
