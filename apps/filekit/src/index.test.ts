...
test("frontmatter stage stages files matching generic frontmatter filters", () => {
  ...
  execFileSync(process.execPath, [cli, "frontmatter", "stage", "--root", root, "--output", output, "--where", "status=published", "visibility=public"]);
...
test("frontmatter stage removes a file when it no longer matches filters", () => {
  ...
  const file = join(root, "essay.md");
  writeFileSync(file, "---\ntitle: Example\nslug: example\ndescription: Example\ntype: essay\nstatus: published\nvisibility: public\n---\n\nBody\n");
  execFileSync(process.execPath, [cli, "frontmatter", "stage", "--root", root, "--output", output, "--where", "status=published", "visibility=public", "--name-field", "slug"]);
