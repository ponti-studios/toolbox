...
pub fn run_essays(opts: EssayClassifyOpts) -> Result<()> {
    let state_dir = opts.dir.join(".filekit").join("classify");
    fs::create_dir_all(&state_dir).context("creating classify state dir")?;

    println!("filekit classify essays");
    ...
    if opts.tui {
        println!("tui mode is not implemented yet in filekit");
        return Ok(());
    }
...
fn pass1_scan(root: &Path) -> Result<Vec<Fingerprint>> {
    ...
    {
        let path = entry.path();
        if path.components().any(|c| c.as_os_str() == ".filekit") {
            continue;
        }
