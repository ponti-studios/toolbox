...
```bash
cargo install --path apps/netkit
```
...
# Custom sites
netkit -s example.com,test.com

# Multiple sites
netkit -s google.com,github.com,stackoverflow.com,reddit.com

# Skip DNS flush
netkit --no-dns-flush

# Custom sites without DNS flush
netkit -s example.com --no-dns-flush

# Single site
netkit -s google.com

# Many sites
netkit -s google.com,facebook.com,twitter.com,linkedin.com,amazon.com

# International sites
netkit -s google.co.uk,google.co.jp,google.com.au

# CDN endpoints
netkit -s cdn.cloudflare.com,cdn.akamai.com

# API endpoints
netkit -s api.github.com,api.openai.com
```
...
```bash
# Build
cargo build -p netkit

# Run from source
cargo run -p netkit -- -s google.com

# Run binary directly
./target/debug/netkit -s google.com,github.com
...
- Source: `apps/netkit/src/main.rs`
- Tests: `apps/netkit/src/main.rs` (inline tests)
