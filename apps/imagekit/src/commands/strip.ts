...
function stripFile(
  src: string,
  tmpDir: string,
  stripper: { tool: string; cmd: string[]; label: string },
): boolean {
...
export function cmdStrip(files: string[], opts: StripOptions): void {
  ...
  if (!stripper) {
    console.error(
      "Error: need exiftool (brew install exiftool) or ImageMagick (brew install imagemagick)",
    );
    process.exit(1);
  }
  ...
  const tmpDir = fs.mkdtempSync("imagekit-strip-");

  console.log("── imagekit strip ───────────────────────────");
