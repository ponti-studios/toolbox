...
function calcCropFromRatio(srcW: number, srcH: number, ratio: { num: number; den: number }): { w: number; h: number } {
  const aspect = ratio.num / ratio.den
...
export function cmdCrop(files: string[], opts: CropOptions): void {
  ...
  for (const src of inputs) {
    ...
    if (!needsMagick) {
      ...
      r = run([
        "magick", src,
        "-gravity", GRAVITY_MAGICK[gravity]!,
