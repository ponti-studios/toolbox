...
export const BIN = path.resolve(import.meta.dir, "../.test-bin/iconkit");
export const APP_DIR = path.resolve(import.meta.dir, "..");
export const FIXTURES = path.resolve(import.meta.dir, "../tests/fixtures");

export function runIconkit(args: string[], env: Record<string, string> = {}) {
...
export function tempDir(label: string): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), `iconkit-${label}-`));
}
