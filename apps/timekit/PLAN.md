...
**`src/timekit_engine/db/models.py`** — Python dataclasses for each table

**`src/timekit_engine/db/queries.py`** — Named query functions (upsert_calendar, upsert_event, mark_deleted, etc.)
...
**`src/timekit_engine/calendar/__init__.py`** — empty

**`src/timekit_engine/calendar/permissions.py`**
...
**`src/timekit_engine/calendar/eventkit.py`**
...
**`src/timekit_engine/calendar/serializers.py`**
...
**`src/timekit_engine/sync/__init__.py`** — empty

**`src/timekit_engine/sync/date_ranges.py`**
...
**`src/timekit_engine/export/__init__.py`** — empty
**`src/timekit_engine/export/ics.py`** — ICS export
**`src/timekit_engine/export/jsonl.py`** — JSONL export
**`src/timekit_engine/export/csv_export.py`** — CSV export
...
1. `pip install -e .` from `apps/timekit/`
2. `timekit doctor` — should report Python version, pyobjc status, permission status, DB status
3. `timekit auth --login` — should trigger macOS calendar permission prompt
4. `timekit sync` — should pull events into SQLite, print summary
5. `timekit preview` — should show upcoming events in Rich table
6. `timekit export --format jsonl` — should write events to file
7. Verify DB at `~/Library/Application Support/timekit/timekit.db` with `sqlite3`
