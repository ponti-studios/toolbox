use crate::error::{GitkitError, Result};
...
impl GitHubClient {
    ...
    pub async fn fetch_file(&self, file: &GitHubFile) -> Result<String> {
        ...
        let mut request = self.client.get(&url).header("User-Agent", "gitkit-cli");
        ...
        let response = request
            .send()
            .await
            .map_err(|e| GitkitError::FetchError(e.to_string()))?;

        let status = response.status().as_u16();
        if status == 404 {
            return Err(GitkitError::ApiError {
                status,
        ...
        if let (Some(content), Some("base64")) = (file_resp.content, file_resp.encoding.as_deref())
        {
            let decoded = base64::engine::general_purpose::STANDARD
                .decode(content.trim())
                .map_err(|e| GitkitError::FetchError(format!("Failed to decode base64: {}", e)))?;
            String::from_utf8(decoded)
                .map_err(|e| GitkitError::FetchError(format!("Failed to decode UTF-8: {}", e)))
        } else if let Some(download_url) = file_resp.download_url {
            let content = self
                .client
                .get(&download_url)
                .header("User-Agent", "gitkit-cli")
                .send()
                .await
                .map_err(|e| GitkitError::FetchError(e.to_string()))?
                .text()
