...
if (values.voices) {
  ...
  console.log("Deepgram Flux voices (use with: openspeek -m deepgram/flux-tts:free -v flux-<voice>-en)");
  console.log("Default: flux-gemma-en (British, Female, Young). Sample clips are the same narration passage.\n");
...
if (engine === "auto") {
  ...
  else if (have("piper") || run("python3", ["-m", "piper", "--help"]).status === 0) engine = "piper";
  else engine = "say";
}
...
async function engineOpenRouter(narration: string): Promise<void> {
  ...
  if (model.includes("flux") && !/^flux-/.test(voice)) {
    console.warn(`⚠ Voice ${voice} looks wrong for Flux; expected "flux-<voice>-en" (see --voices).`);
  }
  ...
  const meta: { rate: number; channels: number }[] = new Array(chunks.length);
  let totalCost = 0;
  ...
  await runPool(chunks, 4, async (text, idx) => {
    const res = await fetch("https://openrouter.ai/api/v1/audio/speech", {
      ...
      body: JSON.stringify(
        speed !== null && /flux|deepgram/i.test(model)
          ? { model, input: text, voice, response_format: "pcm", speed }
    ...
    if (!res.ok) {
      ...
      throw new Error(
        `OpenRouter TTS failed on chunk ${idx + 1}/${chunks.length} (HTTP ${res.status}): ${err}`,
      );
    }
    ...
    meta[idx] = {
      rate: Number(ct.match(/rate=(\d+)/)?.[1] ?? 24000),
      channels: Number(ct.match(/channels=(\d+)/)?.[1] ?? 1),
    };

    const gen = res.headers.get("x-generation-id") ?? "";
    const provider = res.headers.get("x-provider-name") ?? "";
    if (orLogDir && gen) {
      try {
        const d = await fetchGeneration(gen);
        ...
        writeFileSync(join(orLogDir, `${ts.replace(/:/g, "-")}.json`), JSON.stringify(log, null, 2));
      } catch {
...
function enginePiper(narration: string): void {
  ...
  if (r.status !== 0) {
    ...
    const dl =
      ...
      run("pipx", ["run", "--spec", "piper-tts", "python3", "-m", "piper.download_voices", "--data-dir", dataDir, voice]).status === 0;
    if (!dl) {
