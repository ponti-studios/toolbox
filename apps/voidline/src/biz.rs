...
pub fn run_knobs(opts: KnobsOpts) -> Result<()> {
    ...
    let rows = stmt.query_map([model_id], |r| {
        Ok(vec![
            ...
            r.get::<_, String>(4)?,
            r.get::<_, String>(5)?,
            if r.get::<_, i64>(6)? == 1 { "yes".to_string() } else { "no".to_string() },
        ])
    })?;
    let rows: Vec<Vec<String>> = rows.collect::<rusqlite::Result<_>>()?;
    print_table(&["Key", "Label", "Category", "Unit", "Default", "Description", "Advanced"], rows);
    Ok(())
}
...
pub fn run_scenario_list(opts: ListScenarioOpts) -> Result<()> {
    ...
    let rows = if params_vec.is_empty() {
        stmt.query_map([], |r| {
            Ok(vec![
                ...
                r.get::<_, String>(1)?,
                r.get::<_, String>(2)?,
                if r.get::<_, i64>(3)? == 1 { "yes".to_string() } else { "no".to_string() },
    ...
    print_table(&["Model", "Scenario", "Description", "Baseline", "Created", "Updated"], rows);
    Ok(())
}
...
pub fn run_run(opts: RunOpts) -> Result<()> {
    ...
    let (summary, series, resolved) = run_saas_model(&conn, scenario_id, &opts.scenario, opts.months)?;
    save_run(&conn, scenario_id, opts.months, &resolved, &summary, &series)?;
    print_summary(&summary);
    let rows = series.iter().take(12).map(|r| vec![
        r.month.to_string(),
        fmt_num(r.customers),
        fmt_money(r.mrr),
        fmt_money(r.revenue),
    ...
    println!();
    print_table(&["Month", "Customers", "MRR", "Revenue", "Net Income", "Cash End"], rows);
    Ok(())
}
...
pub fn run_compare(opts: CompareOpts) -> Result<()> {
    ...
    print_table(
        ...
        vec![
            vec!["Ending customers".into(), fmt_num(left_summary.ending_customers), fmt_num(right_summary.ending_customers), fmt_num(right_summary.ending_customers - left_summary.ending_customers)],
            vec!["Ending MRR".into(), fmt_money(left_summary.ending_mrr), fmt_money(right_summary.ending_mrr), fmt_money(right_summary.ending_mrr - left_summary.ending_mrr)],
            vec!["Cumulative revenue".into(), fmt_money(left_summary.cumulative_revenue), fmt_money(right_summary.cumulative_revenue), fmt_money(right_summary.cumulative_revenue - left_summary.cumulative_revenue)],
            vec!["Cumulative net income".into(), fmt_money(left_summary.cumulative_net_income), fmt_money(right_summary.cumulative_net_income), fmt_money(right_summary.cumulative_net_income - left_summary.cumulative_net_income)],
            vec!["Ending cash".into(), fmt_money(left_summary.ending_cash), fmt_money(right_summary.ending_cash), fmt_money(right_summary.ending_cash - left_summary.ending_cash)],
            vec!["Break-even month".into(), opt_u32(left_summary.break_even_month), opt_u32(right_summary.break_even_month), compare_optional_months(left_summary.break_even_month, right_summary.break_even_month)],
            vec!["Runway month".into(), opt_u32(left_summary.runway_month), opt_u32(right_summary.runway_month), compare_optional_months(left_summary.runway_month, right_summary.runway_month)],
        ],
    );
    Ok(())
}
...
fn get_db_path(cli_override: &Option<PathBuf>) -> PathBuf {
    ...
    if let Ok(p) = std::env::var("VOIDLINE_DB") {
...
fn seed_saas_model(conn: &Connection) -> Result<()> {
    ...
    conn.execute(
        ...
        params!["saas", "SaaS subscription", "Subscription SaaS scenario model with saved knobs and scenario runs.", now],
    )?;
...
fn resolved_knobs(conn: &Connection, scenario_id: i64) -> Result<BTreeMap<String, String>> {
    ...
    for row in stmt.query_map([model_id], |r| Ok((r.get::<_, String>(0)?, r.get::<_, String>(1)?)))? {
        let (k, v) = row?;
        defaults.insert(k, v);
    }
    let mut stmt = conn.prepare(
    ...
    for row in stmt.query_map([scenario_id], |r| Ok((r.get::<_, String>(0)?, r.get::<_, String>(1)?)))? {
        let (k, v) = row?;
        defaults.insert(k, v);
    }
    Ok(defaults)
...
fn run_saas_model(
    ...
    let effective_arpu = ((1.0 - annual_plan_share) * monthly_price) + (annual_plan_share * (annual_price / 12.0));
    ...
    for month in 1..=months {
        ...
        let operating_profit_before_tax = gross_profit - fixed_monthly_cost - monthly_marketing_spend;
        let taxes = if operating_profit_before_tax > 0.0 { operating_profit_before_tax * tax_rate } else { 0.0 };
...
fn print_summary(summary: &RunSummary) {
    let rows = vec![
        ...
        vec!["Cumulative revenue".into(), fmt_money(summary.cumulative_revenue)],
        vec!["Cumulative net income".into(), fmt_money(summary.cumulative_net_income)],
