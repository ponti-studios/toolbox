...
from chronicle_engine.config import cache_root, db_path, support_root

app = typer.Typer(name="chronicle", help="Calendar intelligence CLI")
console = Console()
...
def auth(
    ...
    if login:
        from chronicle_engine.calendar.permissions import check_access, request_access, CalendarAccess
        ...
        if result == CalendarAccess.AUTHORIZED:
            ...
            from chronicle_engine.calendar.permissions import access_denied_message
            console.print(f"[red]{access_denied_message()}[/red]")
            raise typer.Exit(1)
    else:
        from chronicle_engine.calendar.permissions import check_access
...
def sync():
    ...
    from chronicle_engine.calendar.eventkit import CalendarAccessError
    from chronicle_engine.sync.orchestrator import print_sync_summary, run_sync
    ...
    try:
        ...
        console.print("[dim]Run: chronicle auth --login[/dim]")
        raise typer.Exit(1)
...
def analyze(
    ...
    from chronicle_engine.analysis.runner import print_analysis_summary, run_analysis

    stats = asyncio.run(run_analysis(profile))

    if stats.events_analyzed == 0:
        console.print("[yellow]No events found. Run 'chronicle sync' first.[/yellow]")
        raise typer.Exit(1)
...
def preview():
    ...
    async def _preview():
        ...
        from chronicle_engine.export.common import fetch_canonical_events
...
def export(
    ...
    async def _export():
        if format == "jsonl":
            from chronicle_engine.export.jsonl import export_jsonl
            return await export_jsonl(out_path)
        elif format == "csv":
            from chronicle_engine.export.csv_export import export_csv
            return await export_csv(out_path)
        else:
            from chronicle_engine.export.ics import export_ics
...
def doctor():
    ...
    table = Table(title="Chronicle Health Check")
    ...
    from chronicle_engine.calendar.permissions import check_access, CalendarAccess
    access = check_access()
    if access == CalendarAccess.AUTHORIZED:
        ...
        table.add_row("Calendar Access", "[yellow]PENDING[/yellow]", "Run: chronicle auth --login")
...
def models_pull(
    ...
    from chronicle_engine.models import PROFILES, is_downloaded, model_dir, pull
...
def models_list():
    ...
    from chronicle_engine.models import PROFILES, is_downloaded, model_dir
